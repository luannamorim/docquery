...
- **PII redaction** — CPF, CNPJ, e-mail and BR phone become typed placeholders (`PII_REDACTION_ENABLED`) before anything is embedded, indexed or stored. Not a reopened exclusion but a new obligation: the operational manuals carry real customer data, and an index that re-serves it in citations is an LGPD exposure, not a feature.
- **Emphasis extraction** — yellow/green PDF highlights read as retrieval signal (`EMPHASIS_EXTRACTION_ENABLED`): extra lexical terms and chunk metadata, never rewritten passage text. The operational manuals title their sections and mark critical values with highlights, which no heading regex was going to recover. Red boxes burned into screenshots get the raster half of the same treatment (`IMAGE_EMPHASIS_ENABLED`): OpenCV finds them, the Docling RapidOCR engine reads them, and the contents join the same lexical index.
