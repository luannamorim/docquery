...
def _run_eval(
    label: str, dataset_path: Path, reranker_enabled: bool, n_samples: int = 30
) -> dict:
    ...
    sampled = sample_stratified(items, n=n_samples)

    print(
        f"\n[{label}] Running {len(sampled)} queries"
        f" (reranker={'ON' if reranker_enabled else 'OFF'})..."
    )
...
def main() -> None:
    ...
    with_result = _run_eval(
        "with_reranker", args.dataset, reranker_enabled=True, n_samples=args.samples
    )
    without_result = _run_eval(
        "without_reranker", args.dataset, reranker_enabled=False, n_samples=args.samples
    )
    ...
    summary = {
        ...
        "delta": {
            m: round(
                with_result["scores"].get(m, 0) - without_result["scores"].get(m, 0), 4
            )
            for m in metrics
        },
    ...
    print("\n=== Ablation Summary ===")
    header = (
        f"{'Metric':<25}  {'With reranker':>14}  {'Without reranker':>16}  {'Delta':>8}"
    )
    print(header)
    print("-" * len(header))
    for m in metrics:
    ...
    print(
        f"\n{'cost/query (USD)':<25}"
        f"  {with_result['cost']['mean_cost_per_query_usd']:>14.5f}"
        f"  {without_result['cost']['mean_cost_per_query_usd']:>16.5f}"
        f"  {summary['cost_delta_per_query_usd']:>+8.5f}"
    )
    print(f"\nResults in {out_dir}/")
