...
- Every tokenizer for retrieval goes through `sparse.tokens`. Query side and index side must agree exactly or a term the index holds is not the term the query asks for. It matches `[a-z0-9]+`, so accented words break apart (`política` → `pol`, `tica`) and cannot match an unaccented file name — a known miss, kept because folding accents in one module only would match terms the index does not hold.
