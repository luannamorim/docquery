...
- **A report changes nothing in retrieval — but every asker now sees that it exists.** Flagging a document as outdated (`POST /feedback`) does not change ranking and does not trigger a re-ingest; it exists so a person reviews the document at its source. Its **existence**, though, travels back to every asker as `flagged` on query sources (`_flag_reported` in `routes.py`, `FeedbackStore.reported`) — deliberately reopening the earlier "does not warn other askers" rule, like Docling, Auth and SSE before it. Existence only: comments and counts stay in `GET /feedback`. The lookup is bounded by the caller's sectors like every other feedback read, and a failed lookup logs and degrades to unflagged sources — answering questions must never depend on the feedback database. Reports live in MySQL (`src/docquery/feedback/`), never in the Qdrant payload: re-ingest deletes a source's chunks, and a report must outlive the re-ingest it is asking for.
...
- **A server-flagged card renders the red flag but stays clickable** — a report is per person (upsert on the token's `oid`), so another asker's report must not stop this one from adding their own comment. After *your own* report, every card of the same document goes red and disables together (`markReported` in `ui.ts`): the report covers the document, not the passage. The date line reads `atualização: <data>`, a fact rather than a verdict — "atualizado" sat in contradiction beside a flagged document.
